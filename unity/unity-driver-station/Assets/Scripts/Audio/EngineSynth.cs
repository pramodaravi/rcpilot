...
namespace RcPilot.Audio
{
    ...
    public class EngineSynth : MonoBehaviour
    {
        ...
        private byte _state = TelemetryStates.STATE_IDLE;
        ...
        private void Update()
        {
            ...
            float gate = 1f;
            if (_state == TelemetryStates.STATE_IDLE)  gate = 0.25f;
            if (_state == TelemetryStates.STATE_ESTOP) gate = 0.1f;
            if (_state == TelemetryStates.STATE_FAULT) gate = 0.0f;
