@echo off
rem Stitched-camera bridge launcher for the Windows cockpit machine.
rem RTP UDP 5004 -> raw RGB TCP 9000 (fast preset). Matches
...
set SCRIPT_DIR=%~dp0
set PY=py -3.11
echo [bridge.bat] starting merged feed (RTP auto:5004 -> raw RGB TCP 127.0.0.1:9000, fast preset)
%PY% "%SCRIPT_DIR%bridge.py" --in-port 5004 --out-port 9000 --bind 127.0.0.1 --rtp-bind auto --preset fast --frame-format raw
endlocal
