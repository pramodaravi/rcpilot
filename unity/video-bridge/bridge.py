"""RTP H.264 → JPEG TCP bridge for the Unity driver station (macOS port).

The Jetson sends RTP H.264 to UDP 5000/5001 (cam0/cam1). Unity's built-in
H.264 decoding is flaky for live low-latency RTP, so this sidecar:
...
Run two copies, one per camera:

    python bridge.py --in-port 5000 --out-port 9000
    python bridge.py --in-port 5001 --out-port 9001

Or use bridge.sh which launches both in the background.
...
def main() -> int:
    ...
    p.add_argument("--in-port", type=int, default=5000,
                   help="RTP UDP port to receive on")
