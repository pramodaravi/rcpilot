...
namespace RcPilot.Core
{
    ...
    public class Bootstrapper : MonoBehaviour
    {
        ...
        public TelemetryReceiver telemetry;
        public VideoBridgeClient cam0;
        public VideoBridgeClient cam1;
        ...
        private IEnumerator Start()
        {
            ...
            // Network — control sender and telemetry receiver are always created
            // so the rest of the app can reference them without null checks. In
            // sim mode they're left un-Configured (ControlSender.Ready stays
            // false, TelemetryReceiver opens no socket) and SimTelemetryBridge
            // injects synthetic packets directly.
            ...
            if (!config.sim.enabled)
            {
                ...
                telemetry.Configure(config.network.telemetryPort);
            }
            else
            {
                Log.Info("SIM MODE: skipping ControlSender/TelemetryReceiver socket setup");
            }
            ...
            if (!config.sim.enabled)
            {
                // Real video bridges — one per camera. Fine if the bridges aren't
                // running yet; they auto-reconnect.
        ...
        private void OnApplicationQuit()
        {
            ...
            if (controlSender != null) controlSender.SendDisarmAndQuit();
        }
